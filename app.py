...
# 2. API Key Configuration (Free Tier via Google AI Studio)
# In production, use st.secrets. For testing, you can paste it in the sidebar.
...
with tab1:
    ...
    if uploaded_files and not api_key:
        st.error("Please add your Gemini API Key in the sidebar to parse these resumes!")
                ...
                with st.spinner(f"Processing {uploaded_file.name}..."):
                    try:
                        ...
                        {{
                            ...
                            "total_experience_years": 0.0 (as a float),
                        ...
                        # Standardize text to clean potential markdown wrappers if LLM hallucinates them
                        clean_json = response.text.strip().replace("```json", "").replace("```", "")
                        candidate_data = json.loads(clean_json)
                        
                        # Add operational defaults that resumes don't usually have
...
with tab2:
    ...
    if not st.session_state.candidate_db:
        ...
        # Convert our database to a DataFrame for easy editing via Streamlit's Data Editor
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "expected_budget_lpa": st.column_config.NumberColumn("Expected Budget (LPA)", format="%.2f f"),
                ...
                if not filtered_candidates:
                    ...
                    try:
                        ...
                        for rank, match in enumerate(match_results, 1):
                            ...
                            # Custom color coding for matching scores
                            if score >= 80:
                                score_color = "🟢"
                            elif score >= 50:
                                score_color = "🟡"
